	...
	LPSTR lpCmdLine, int nCmdShow)
{
	...
	// モデルのロード
	int hndl1 = MV1LoadModel("Data/Models/HitBox/BoxChan.x");
	int hndl2 = MV1DuplicateModel(hndl1);
	...
	while (ProcessMessage() != -1)
	{
		...
		// 当たり判定結果を表示
		VECTOR size = { 2.0f, 3.0f, 2.6f };
		bool isHit = CheckHitBoxToBox(pos1, size, pos2, size);
		if (isHit == true)
		{
